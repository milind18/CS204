...
Description: The data structure used to store all the variables is a trie. This data structure performs insertions and lookups of variables both in O(1) time! (more correctly O(k) time where k is the maximum possible length of the string which is given to be 15).
Each node of the trie represents a string S and contains pointers to nodes whose strings have S as a prefix. 
